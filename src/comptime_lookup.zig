...
pub fn AutoComptimeLookup(comptime K: type, comptime V: type, comptime kvs: anytype) type {
    return ComptimeLookup(K, V, std.hash_map.getAutoEqlFn(K), kvs);
}

pub fn ComptimeLookup(comptime K: type, comptime V: type, comptime eql: fn (a: K, b: K) bool, comptime kvs: anytype) type {
    return struct {
